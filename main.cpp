...
void showFastAccountList(AccountManager &manager, const string &currentUser, AppLanguage lang)
{
	...
	if (lang == AppLanguage::Ukrainian)
		cout << "Доступні рахунки (ID | Назва | Баланс):\n";
	else
		cout << "Available accounts (ID | Name | Balance):\n";
		...
		else if (choice == 1)
		{
			...
			for (const auto &acc : accounts)
			{
				if (acc->hasAccess(currentUser))
				{
					...
					if (acc->isShared())
						cout << ((lang == AppLanguage::Ukrainian) ? " (Спільний)" : " (Shared)");
					cout << "\n";
				}
			}
			if (!foundAny)
				cout << ((lang == AppLanguage::Ukrainian) ? "У вас немає доступних рахунків.\n" : "You have no accessible accounts.\n");
			waitUser();
		}
		else if (choice == 2)
		{
		...
		else if (choice == 3 || choice == 4)
		{
			...
			while (true)
			{
				...
				if (selectedAcc != nullptr)
				{
					...
					cout << ((lang == AppLanguage::Ukrainian) ? "Рахунок не знайдено! Спробуйте ще раз.\n" : "Account not found! Try again.\n");
			}
			...
			if (success)
			{
				...
				cout << ((lang == AppLanguage::Ukrainian) ? "-> Помилка! Недостатньо коштів.\n" : "-> Error! Insufficient funds.\n");
			waitUser();
		}
		else if (choice == 5)
		{
